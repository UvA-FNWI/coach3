...
def has_access(email, name):
    ...
    if email.lower() in access_df['email'].values:
        student_rights = access_df[access_df['email']==email.lower()]
        if student_rights['type'].values == 'test':
            return True
    elif name in access_df['name'].values:
        student_rights = access_df[access_df['name']==name]
        if student_rights['type'].values == 'test':
            return True
    return  False

def can_use_data(name):
    """
    ...
    #         return True
    if name in access_df['name'].values:
        student_access = access_df[access_df['name']==name]
        if student_access['access'].values == 'y':
            return True
    return False